...
bale_pending = {}
sent_keys    = set()
sent_keys_lock = threading.Lock()
bale_sent_map = {}  # tg_source_key → bale_channel_msg_id (for reply tracking)
_counter     = 0
...
def send_file_retry3(method, file_path, field_name, mime, payload):
    """Try sending file up to 3 times. Returns True on success."""
    for attempt in range(3):
        if send_file_to_bale(method, file_path, field_name, mime, payload):
            return True
        log.warning("Bale %s attempt %d/3 failed", method, attempt + 1)
        if attempt < 2:
            time.sleep(3)
    return False

def notify_admin_media_failed(entry):
    """Notify admins that media could not be sent."""
    src = SOURCE_NAMES.get(entry.get("source_username", ""), entry.get("source_username", "ناشناس"))
    preview = entry.get("caption", "").split("\n\n🇮🇷")[0].strip()[:200]
    text = (
        f"⚠️ *ارسال رسانه به کانال بله ناموفق بود*\n\n"
        f"*منبع:* {src}\n"
        f"*پیش‌نمایش:*\n{preview}\n\n"
        f"خبر بدون تصویر/ویدیو ارسال نشد."
    )
    for admin_id in BALE_ADMIN_IDS:
        api("sendMessage", chat_id=admin_id, text=text, parse_mode="Markdown")

def send_to_channel(entry):
    mtype      = entry["type"]
    caption    = bold_first_para(entry.get("caption", ""))
    media_paths = entry.get("media_paths", [])  # list of (path, mime)
    if not media_paths and entry.get("media_path"):
        media_paths = [(entry["media_path"], entry.get("mime", ""))]

    reply_to = entry.get("bale_reply_to_msg_id")
    ...
    try:
        if mtype == "text" or not media_paths:
            msg_payload = {"chat_id": BALE_TARGET_CHANNEL, "parse_mode": "Markdown",
                          "text": caption + BALE_FOOTER}
            if reply_to:
                msg_payload["reply_to_message_id"] = reply_to
            result = api("sendMessage", **msg_payload)
            if result:
                log.info("Sent text to Bale channel")
            return

        # Try to send all media files
        first_sent = False
        for i, (media_path, mime) in enumerate(media_paths):
            if not media_path or not os.path.exists(media_path):
                continue

            file_payload = dict(payload)
            if i > 0:
                # Subsequent files: no caption, no reply
                file_payload.pop("caption", None)
                file_payload.pop("reply_to_message_id", None)

            if mime.startswith("image/") or mtype in ("photo", "album_photo"):
                ok = send_file_retry3("sendPhoto", media_path, "photo", "image/jpeg", file_payload)
            elif mime.startswith("video/") or mtype in ("video", "album_video"):
                size = os.path.getsize(media_path)
                if size > BALE_MAX_VIDEO_BYTES:
                    log.info("Skipped video > 20MB")
                    continue
                ok = send_file_retry3("sendVideo", media_path, "video", "video/mp4",
                                     {**file_payload, "supports_streaming": "true"})
            else:
                ok = send_file_retry3("sendDocument", media_path, "document",
                                     mime or "application/octet-stream", file_payload)

            if ok:
                first_sent = True
                log.info("Sent media %d/%d to Bale channel", i+1, len(media_paths))
            else:
                log.warning("Media %d/%d failed", i+1, len(media_paths))

        if not first_sent:
            # All media failed — notify admin, don't send broken post
            log.error("All media uploads failed — notifying admin")
            notify_admin_media_failed(entry)
            return
...
def _process_one_record(record) -> dict | None:
    """Process a single queue record. Downloads all media. Returns entry or None."""
    ...
    if tg_chat_id and tg_msg_ids and mtype != "text":
        for msg_id in tg_msg_ids[:10]:  # max 10 files
            path, mime = download_media_to_disk(tg_chat_id, msg_id)
            if path:
                media_paths.append((path, mime))

    # Reply support — look up if source replied to a message we already sent
    bale_reply_to = None
    tg_reply_to_id = record.get("tg_reply_to_msg_id")
    if tg_reply_to_id and tg_chat_id:
        reply_key = f"{tg_chat_id}_{tg_reply_to_id}"
        bale_reply_to = bale_sent_map.get(reply_key)
    ...
    entry = {
        ...
        "media_path":           media_paths[0][0] if media_paths else None,
        "mime":                 media_paths[0][1] if media_paths else "",
        "bale_reply_to_msg_id": bale_reply_to,
        "tg_source_key":        f"{tg_chat_id}_{tg_msg_ids[0]}" if tg_msg_ids else None,
    }
    return entry
...
def process_queue(seen):
    ...
    for line in lines:
        ...
        try:
            ...
            if time.time() - record.get("ts", 0) > 21600:  # 6 hours
                seen.add(line_key)
                continue
            entry = _process_one_record(record)
            if bale_mode == "auto":
                send_to_channel(entry)
                for path, _ in entry.get("media_paths", []):
                    delete_media(path)
            else:
                ask_approval(entry)
            seen.add(line_key)
    ...
    for line in lines:
        ...
        try:
            ...
            if age > 21600:  # 6 hours
                log.info("Queue: skipping expired item (age=%.0fs)", age)
                seen.add(line_key)
                _save_seen(seen)
                return seen
            log.info("Queue: processing item type=%s src=%s age=%.0fs",
                     record.get("type"), record.get("source_username"), age)
            entry = _process_one_record(record)
            if bale_mode == "auto":
                send_to_channel(entry)
                for path, _ in entry.get("media_paths", []):
            ...
            log.warning("Queue process error: %s | %s", e, line[:80])
            seen.add(line_key)
    return seen
    ...
    for attempt in range(10):
        try:
            ...
            if "database is locked" in str(e) and attempt < 9:
                log.warning("Session locked, retrying in 3s... (%d/10)", attempt+1)
                time.sleep(3)
                try:
                    tg.disconnect()
                except:
